 ...
 ******************************************************************************/
static uint8_t u8Senddata[10] = {0x12,0x34,0x77,0x66,0x55,0x44,0x33,0x22,0xEE,0xDD};
...
int32_t main(void)
{
    ...
    ///< eeprom写数据
    I2C_MasterWriteData(M0P_I2C,0x10,u8Senddata,10);  
    delay1ms(100);
    
    ///< eeprom读数据
    I2C_MasterReadData(M0P_I2C,0x10,u8Recdata,10);    
...
void App_PortCfg(void)
{
    ...
    Gpio_Init(EVB_I2C_EEPROM_SDA_PORT, EVB_I2C_EEPROM_SDA_PIN,&stcGpioCfg); ///< 初始化PA12
    
    Gpio_SetAfMode(EVB_I2C_EEPROM_SCL_PORT, EVB_I2C_EEPROM_SCL_PIN,GpioAf6);///< 配置PA11为SCL
    Gpio_SetAfMode(EVB_I2C_EEPROM_SDA_PORT, EVB_I2C_EEPROM_SDA_PIN,GpioAf6);///< 配置PA12为SDA    
}
...
void App_I2cCfg(void)
{
    ...
    Sysctrl_SetPeripheralGate(SysctrlPeripheralI2c,TRUE);  ///< 开启I2C时钟门控   
